...
export function createNpcStatBlockSchema(options: StatBlockSchemaOptions) {
  ...
  return z
    .object({
      ...
      defensiveAbilities: z.string().max(1000).optional(),
      damageReduction: z.string().max(500).optional(),
      spellResistance: z.string().max(500).optional(),
      weaknesses: z.string().max(500).optional(),
      feats: z.string().max(5000).optional(),
