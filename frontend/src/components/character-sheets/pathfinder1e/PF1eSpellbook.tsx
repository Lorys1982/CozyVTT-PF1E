...
import { spellAoEFromPf1e, type SpellAoEConfig } from '../../../utils/pathfinder1eSpellAoE';
...
function SpellCard({spell,aoeConfig,editable,canTrackUses,castingType,slotsRemaining,onChange,onRemove,onPrepare,onUnprepare,onCast,onRestore,onPlaceAoE}:{spell:PF1eSpell;aoeConfig?:SpellAoEConfig;editable:boolean;canTrackUses:boolean;castingType:'prepared'|'spontaneous';slotsRemaining:number;onChange:(spell:PF1eSpell)=>void;onRemove:()=>void;onPrepare:()=>void;onUnprepare:()=>void;onCast:()=>void;onRestore:()=>void;onPlaceAoE?:(config:SpellAoEConfig,spell:PF1eSpell)=>void}) {
  ...
  return (
    <article onClick={!editable&&!expanded?()=>setExpanded(true):undefined} className={`rounded-xl border border-stone-200 bg-white p-4 shadow-sm ${!editable?'cursor-pointer transition hover:border-purple-300 hover:shadow-md':''}`}>
      ...
      {aoeConfig&&onPlaceAoE&&<div className="mt-3" onClick={event=>event.stopPropagation()}><button type="button" onClick={()=>onPlaceAoE(aoeConfig,spell)} className="inline-flex items-center gap-2 rounded-lg border border-purple-300 bg-purple-50 px-3 py-2 text-sm font-bold text-purple-900 hover:bg-purple-100"><Crosshair className="h-4 w-4"/>Place {aoeConfig.shape==='sphere'?'circle':aoeConfig.shape} · {aoeConfig.sizeFt} ft{aoeConfig.shape==='line'?` × ${aoeConfig.widthFt??5} ft`:''}</button></div>}
