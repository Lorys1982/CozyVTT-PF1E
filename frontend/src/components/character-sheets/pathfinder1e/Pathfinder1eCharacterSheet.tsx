...
import {
  Backpack, BookOpen, Heart, NotebookPen, Package, Save, Shield, Sparkles,
  Swords, Trash2, UserRound, X,
} from 'lucide-react';
      ...
      {items.map((attack,index)=>{
        ...
        return <article key={index} className="rounded-xl border border-stone-200 bg-white p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <button type="button" disabled={!onRoll||!attack.attackBonus} onClick={()=>{const bonus=attack.attackBonus?.match(/[+-]?\d+/)?.[0]??'+0';onRoll?.(`1d20${Number(bonus)>=0?'+':''}${Number(bonus)}`,`${attack.weapon||title} Attack`)}} className="rounded-lg bg-red-800 px-4 py-3 text-left text-white disabled:cursor-default"><span className="block text-xs uppercase text-red-200">Attack</span><strong className="text-xl">{attack.attackBonus||'—'}</strong></button>
            <button type="button" disabled={!onRoll||!attack.damage} onClick={()=>attack.damage&&onRoll?.(attack.damage,`${attack.weapon||title} Damage`)} className="rounded-lg bg-stone-800 px-4 py-3 text-left text-white disabled:cursor-default"><span className="block text-xs uppercase text-stone-300">Damage</span><strong className="text-xl">{attack.damage||'—'}</strong></button>
          </div>
        </article>;
      })}
  ...
  return <div className="space-y-6">
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Panel title="Combat Numbers" icon={Swords}>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><SheetInput label="Base attack bonus" value={data.bab} editable={editable} type="number" onChange={value=>onSet('bab',value)} /><SheetInput label="Initiative misc" value={data.initiative?.miscModifier} editable={editable} type="number" onChange={value=>onSet('initiative.miscModifier',value)} /><SheetInput label="Initiative temporary" value={data.initiative?.tempModifier} editable={editable} type="number" onChange={value=>onSet('initiative.tempModifier',value)} /><SheetInput label="Initiative override" value={data.initiative?.overrideTotal} editable={editable} type="number" placeholder="Automatic" onChange={value=>onSet('initiative.overrideTotal',value)} /><SheetInput label="Base speed" value={data.speed?.base} editable={editable} placeholder="30 ft." onChange={value=>onSet('speed.base',value)} /><SheetInput label="Speed temporary / notes" value={data.speed?.tempModifiers} editable={editable} onChange={value=>onSet('speed.tempModifiers',value)} /></div>
...
function GearList({data,editable,onSet}:{data:PF1eCharacterData;editable:boolean;onSet:(path:string,value:unknown)=>void}) {
  ...
  return <div className="space-y-3">{gear.map((item,index)=><div key={index} className="grid gap-3 rounded-xl border border-stone-200 bg-white p-3 sm:grid-cols-2 lg:grid-cols-6"><div className="lg:col-span-2"><SheetInput label="Item" value={item.name} editable={editable} onChange={value=>update(index,'name',value)} /></div><SheetInput label="Quantity" value={item.quantity} editable={editable} type="number" min={0} onChange={value=>update(index,'quantity',value)} /><SheetInput label="Type" value={item.type} editable={editable} onChange={value=>update(index,'type',value)} /><SheetInput label="Location" value={item.location} editable={editable} onChange={value=>update(index,'location',value)} /><div className="flex items-end gap-2"><div className="flex-1"><SheetInput label="Weight" value={item.weight} editable={editable} onChange={value=>update(index,'weight',value)} /></div>{editable&&<button type="button" aria-label={`Remove ${item.name||'item'}`} onClick={()=>onSet('gear',gear.filter((_,itemIndex)=>itemIndex!==index))} className="mb-2 text-stone-400 hover:text-red-700"><Trash2 className="h-4 w-4" /></button>}</div></div>)}{editable&&<button type="button" onClick={()=>onSet('gear',[...gear,{name:'',quantity:1}])} className="rounded-lg bg-red-800 px-3 py-2 text-sm font-semibold text-white">+ Item</button>}</div>;
}
...
function FeatureList({title,values,editable,onChange}:{title:string;values:PF1eFeature[];editable:boolean;onChange:(values:PF1eFeature[])=>void}) {
  ...
  return <Panel title={title}><div className="grid gap-3 md:grid-cols-2">{values.map((feature,index)=><article key={index} className="rounded-xl border border-stone-200 bg-white p-4">{editable?<><div className="flex gap-2"><input aria-label={`${title} name`} value={feature.name} onChange={event=>update(index,'name',event.target.value)} placeholder="Name" className={`${inputClass} font-semibold`} /><input aria-label={`${title} type`} value={feature.type??''} onChange={event=>update(index,'type',event.target.value)} placeholder="Type" className={`${inputClass} max-w-32`} /><button type="button" aria-label={`Remove ${feature.name||title}`} onClick={()=>onChange(values.filter((_,itemIndex)=>itemIndex!==index))} className="text-stone-400 hover:text-red-700"><Trash2 className="h-4 w-4" /></button></div><textarea aria-label={`${title} description`} value={feature.description??''} onChange={event=>update(index,'description',event.target.value)} placeholder="Rules text or notes" className={`${inputClass} mt-3 min-h-24 resize-y`} /></>:<><div className="font-bold text-stone-900">{feature.name}{feature.type&&<span className="ml-2 rounded-full bg-red-50 px-2 py-0.5 text-xs font-semibold text-red-800">{feature.type}</span>}</div>{feature.description&&<div className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-stone-600">{feature.description}</div>}</>}</article>)}{!values.length&&<div className="col-span-full rounded-xl border-2 border-dashed border-stone-200 py-8 text-center text-sm text-stone-500">Nothing recorded yet.</div>}</div>{editable&&<button type="button" onClick={()=>onChange([...values,{name:''}])} className="mt-3 rounded-lg bg-red-800 px-3 py-2 text-sm font-semibold text-white">+ {title}</button>}</Panel>;
}
...
export default function Pathfinder1eCharacterSheet({character,mode,onSave,onCancel,onRoll}:Pathfinder1eCharacterSheetProps) {
  ...
  const save=async()=>{if(!onSave)return;setSaving(true);try{await onSave(preparePF1eDataForSave(data),true);}finally{setSaving(false);}};
  ...
  return <div className="mx-auto max-w-[1180px] overflow-hidden rounded-xl border border-stone-200 bg-white text-stone-900 shadow-xl">
    <header className="relative bg-gradient-to-r from-red-700 via-red-800 to-red-950 p-6 text-white">
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4">{character.tokenImageUrl?<img src={character.tokenImageUrl} alt="" className="h-24 w-24 rounded-full border-4 border-white/20 object-cover" />:<div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-4 border-white/20 bg-black/20"><Swords className="h-9 w-9 text-white/70" /></div>}<div className="min-w-0">{editable?<input aria-label="Character name" value={data.characterName} onChange={event=>set('characterName',event.target.value)} className="w-full min-w-0 rounded-lg border border-white/20 bg-white/10 px-3 py-1 text-3xl font-black text-white placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-white/40" />:<h2 className="truncate text-3xl font-black">{data.characterName}</h2>}<div className="mt-2 flex flex-wrap gap-2">{editable?<><input aria-label="Class and level" value={data.classAndLevel??''} onChange={event=>set('classAndLevel',event.target.value)} placeholder="Class & level" className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm text-white placeholder:text-white/60" /><input aria-label="Race summary" value={data.race??''} onChange={event=>set('race',event.target.value)} placeholder="Race" className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm text-white placeholder:text-white/60" /></>:<><span className="rounded-full bg-white/15 px-3 py-1 text-sm font-medium">{data.classAndLevel||'Adventurer'}</span>{data.race&&<span className="rounded-full bg-white/10 px-3 py-1 text-sm">{data.race}</span>}{data.alignment&&<span className="rounded-full bg-white/10 px-3 py-1 text-sm">{data.alignment}</span>}</>}</div></div></div>
