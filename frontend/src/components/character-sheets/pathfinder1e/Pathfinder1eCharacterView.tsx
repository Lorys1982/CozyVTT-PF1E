...
function SpeedInfo({speed}:{speed:PF1eCharacterData['speed']}) {
  const entries = [['Base', speed?.base], ['Armor', speed?.withArmor], ['Fly', speed?.fly], ['Swim', speed?.swim], ['Climb', speed?.climb], ['Burrow', speed?.burrow]]
    .filter(([, value]) => value);
  return <Info label="Speed" value={entries.length ? <span className="flex flex-wrap gap-x-3 gap-y-1">{entries.map(([label, value]) => <span key={label}><span className="text-xs text-stone-500">{label}:</span> {value}</span>)}</span> : undefined} />;
}
...
export default function Pathfinder1eCharacterView({character,onEdit,onRoll,onDataChange,onPlaceAoE}:Props) {
  ...
  return <div className="mx-auto max-w-[1180px] overflow-hidden rounded-xl border border-stone-200 bg-white text-stone-900 shadow-xl">
    ...
    <main className={`pf1-sheet-main space-y-6 p-4 sm:p-6 ${layout==='tabs'?`pf1-tabs-${activeTab}`:''}`}>
      ...
      <Section title="Combat" icon={Shield}><div className="grid grid-cols-2 gap-3 sm:grid-cols-5"><Info label="BAB" value={signed((data.bab??0)+(data.babMiscModifier??0))}/><Rollable expression={`1d20${signed(data.cmb?.total??0)}`} purpose="CMB" onRoll={onRoll} onMenu={showRollMenu} className="rounded-lg bg-white px-3 py-2 text-left hover:bg-red-50"><div className="text-[10px] font-bold uppercase text-stone-500">CMB</div><div className="font-black">{signed(data.cmb?.total??0)}</div></Rollable><Info label="CMD" value={data.cmd?.total??10}/><SpeedInfo speed={data.speed}/><Info label="Damage reduction" value={data.damageReduction}/></div></Section>
